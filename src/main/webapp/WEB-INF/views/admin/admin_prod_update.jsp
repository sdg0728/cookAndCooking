...
<head>
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath }/resources/purchase/CSS/admin_prod_update.css">
...
<body>
    ...
    <section>
        <div class="content-wrap">
            <h3 class="black">상품 수정</h3>
            <form name="updateFrom" action="${pageContext.request.contextPath }/admin/updateOk" method="POST" enctype="multipart/form-data">
            	<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
            	<input type="hidden" name="pid" value="${dto.pid }"/>
                <div class="prod-info-wrap">
                    <div class="prod-img">
                        <img src="${pageContext.request.contextPath }/upload/recipe/${dto.p_img}" />
                        <input id="up-file" class="hidden" type="file" name="upload" accept=".gif, .jpg, .png, .jpeg" required/>
                    </div>
                    <div class="prod-info">
                        <div class="info">
                            <div class="prod-name black">
                                <div class="name">${dto.p_name }</div>
                            </div>
                            <div class="add-prod-price black">
                               	<div>가격</div>
                               	<div class="price">${dto.p_price }</div>
                            </div>
                            <div class="add-prod-kind black">
                            	<div>상품 분류</div>
                                <div class="kind">${dto.p_kind }</div>
                            </div>
                            <div class="add-prod-taste black">
                            	<div>조리법</div>
                                <div class="taste">${dto.p_taste }</div>
                            </div>
                            <div class="form-group a-prod-content black">
                                <div>상품 설명</div>
                                <div class="a-content">${dto.p_content }</div>
                            </div>
                        </div>
                        <div class="form-group add-prod-count black">
                            <div>수량</div>
                            <div class="count">${dto.p_count }</div>
                        </div>
                        <div class="btn-wrap">
                        	<button type="button" name="update" class="btn btn-dark btn-update" data-status="0">수정하기</button>
                        </div>
                    </div>
                </div>
            </form>
    ...
    <script src="${pageContext.request.contextPath }/resources/purchase/JS/admin_prod_update_ajax.js"></script>
